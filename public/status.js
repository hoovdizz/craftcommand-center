...
function renderAttachment(att) {
  ...
  box.innerHTML = att?.method === 'rcon'
    ? `<div><strong>Status:</strong> ${att?.ok ? 'Authenticated' : 'Not connected'}</div><div><strong>RCON endpoint:</strong> <code>${escapeHtml(att?.endpoint || 'unknown')}</code></div><div><strong>Game endpoint:</strong> <code>${escapeHtml(att?.gameEndpoint || 'unknown')}</code></div><div><strong>Method:</strong> <code>RCON</code></div><div><strong>Last check:</strong> ${escapeHtml(formatTime(att?.checkedAt))}</div>${att?.error ? `<div><strong>Error:</strong> ${escapeHtml(att.error)}</div>` : ''}`
    : `<div><strong>Status:</strong> ${att?.ok ? 'Ready' : 'Not attached'}</div><div><strong>Container:</strong> <code>${escapeHtml(att?.container || 'unknown')}</code></div><div><strong>Docker user:</strong> <code>${escapeHtml(att?.dockerUser || 'unknown')}</code></div><div><strong>Configured screen:</strong> <code>${escapeHtml(att?.configuredScreenSession || 'auto')}</code></div><div><strong>Active screen:</strong> <code>${escapeHtml(att?.activeScreenSession || 'not found')}</code></div><div><strong>Method:</strong> <code>${escapeHtml(att?.method || 'attach')}</code></div><div><strong>Last check:</strong> ${escapeHtml(formatTime(att?.checkedAt))}</div>${att?.error ? `<div><strong>Error:</strong> ${escapeHtml(att.error)}</div>` : ''}`;
}
function renderConnection(connection) {
  $('#connectionMode').value = connection.mode || 'binhex'; $('#connectionHost').value = connection.host || '';
  $('#connectionGamePort').value = connection.gamePort || 19132; $('#connectionRconPort').value = connection.rconPort || 25575;
  $('#connectionPassword').placeholder = connection.passwordConfigured ? 'Saved (leave blank to keep)' : 'RCON password';
}
...
async function init() {
  ...
  if (can('admin')) { renderConnection(config.connection || {}); $('#backupCard').classList.toggle('hidden', config.connection?.mode === 'rcon'); }
}
$('#connectionForm')?.addEventListener('submit', async event => { event.preventDefault(); const button = event.submitter; button.disabled = true; try { const body = { mode: $('#connectionMode').value, host: $('#connectionHost').value.trim(), gamePort: Number($('#connectionGamePort').value), rconPort: Number($('#connectionRconPort').value), rconPassword: $('#connectionPassword').value }; const data = await request('/api/connection/save', { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify(body) }); renderConnection(data.connection); renderAttachment(data.attachment); $('#connectionPassword').value = ''; setResult(data.connection.mode === 'rcon' ? 'RCON authentication succeeded.' : 'Binhex connection mode saved.'); } catch (error) { setResult(`Connection failed: ${error.message}`); } finally { button.disabled = false; } });
$('#scanLan')?.addEventListener('click', async event => { const button = event.currentTarget; button.disabled = true; $('#lanServers').innerHTML = '<p class="hint">Scanning the local network...</p>'; try { const data = await request('/api/connection/scan', { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify({ port: Number($('#connectionGamePort').value) }) }); $('#lanServers').innerHTML = data.servers.length ? data.servers.map(server => `<article class="accessEntry"><strong>${escapeHtml(server.motd)} — ${escapeHtml(server.host)}:${server.port}</strong><span>${escapeHtml(server.version || 'Unknown version')} • ${server.onlinePlayers}/${server.maxPlayers} players</span><button type="button" class="compact" data-use-host="${escapeHtml(server.host)}">Use server</button></article>`).join('') : '<p class="hint">No Bedrock servers replied on that port. Check the port and Docker host network access.</p>'; $('#lanServers').querySelectorAll('[data-use-host]').forEach(choice => choice.addEventListener('click', () => { $('#connectionHost').value = choice.dataset.useHost; $('#connectionMode').value = 'rcon'; })); setResult(`LAN scan found ${data.servers.length} server(s).`); } catch (error) { $('#lanServers').innerHTML = ''; setResult(`LAN scan failed: ${error.message}`); } finally { button.disabled = false; } });
